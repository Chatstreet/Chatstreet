...
def send_verification_mail(recipient: str, verification_code: str, username: str, user_tag: int) -> bool:
    from app import conf
    if not is_valid_email(recipient):
        return False
    title: str = 'Verify Your E-Mail'
    template: str = 'mail/verification.html'
    url: str = f'http://{conf.FRONTEND_BASE_URL}/email-verification/{verification_code}'
    msg = Message(title, recipients=[recipient])
    msg.html = render_template(template, email=recipient, link=url, user='{username}#{user_tag}'.format(username=username, user_tag=user_tag))
    return send(msg)
...
def send_password_reset_mail(recipient: str, username: str, reset_code: str) -> bool:
    ...
    url: str = f'http://{conf.FRONTEND_BASE_URL}/reset-password/{reset_code}'
    msg = Message(title, recipients=[recipient])
